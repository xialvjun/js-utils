...
function args_parallel_limit(fn, count) {
    ...
    return function () {
        ...
        var args_fn = args_fns.find(function (it) {
            return it.args.length === args.length &&
                it.args.every(function (arg, i) { return (arg = args[i]); });
        });
        if (args_fn) {
            return args_fn.fn.apply(args_fn, args);
        }
